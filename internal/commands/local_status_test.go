...
func TestLocalStatus(t *testing.T) {
	...
	lru := time.Date(2025, time.November, 17, 17, 12, 57, 524467000, time.UTC).Local().Format(time.RFC3339)
	mru := time.Date(2025, time.November, 17, 17, 13, 7, 284400000, time.UTC).Local().Format(time.RFC3339)
	...
	t.Run("text", func(t *testing.T) {
		...
		shoulda.SatisfyWith(t, actual, "Least recently used: "+lru, strings.Contains)
		shoulda.SatisfyWith(t, actual, "Most recently used: "+mru, strings.Contains)
	...
	t.Run("JSON", func(t *testing.T) {
		...
		shoulda.BeEqual(t, *got.Cache.LeastRecentlyUsed, lru)
		shoulda.BeEqual(t, *got.Cache.MostRecentlyUsed, mru)
